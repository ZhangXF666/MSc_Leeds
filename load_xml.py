...
def read_data():
    ...
    Data['RCL'] = {}
    tree = ET.parse('./data/treebank/losr.xml')
    root = tree.getroot()
    for child in root:
    ...
    tree = ET.parse('./data/treebank/commands.xml')
    root = tree.getroot()
    Data[root.tag] = {}  # Data['commands']
    Data['commands_id'] = {}
    for child in root:
    ...
    tree1 = ET.parse('./data/treebank/comments.xml')
    root1 = tree1.getroot()
    Data['comments'] = {}
    for child in root1:
        Id = int(child.attrib['id'])
    ...
    Data['layouts'] = {}
    Data['gripper'] = {}
    tree = ET.parse('./data/treebank/layouts.xml')
    root = tree.getroot()
    for child in root:
        counter = 0
        Id = int(child.attrib['id'])
    ...
    Data['scenes'] = {}
    tree = ET.parse('./data/treebank/scenes.xml')
    root = tree.getroot()
    for child in root:
        Id = int(child.attrib['id'])
        Data['scenes'][Id] = {}
